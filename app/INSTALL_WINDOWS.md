...
Run packaging commands from the top-level `app/` folder (the one containing `INSTALL_WINDOWS.md`, `release-readiness.ps1`, and `package.json`).

If you cloned/extracted into nested folders, ensure there is **no second `app/app` project folder** before running builds.
